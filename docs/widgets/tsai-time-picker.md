...
`showTsaiTimePicker` opens a content-sized `Select time` sheet with Cancel /
Apply. `TsaiTimeField` is the input-styled control that opens that sheet.
`minuteStep` may be 1, 5, or 15.
